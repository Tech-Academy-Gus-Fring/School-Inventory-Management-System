...
import { Package, ClipboardList, CalendarClock, LogOut, Search, SendHorizontal } from 'lucide-react';
import { ThemeToggle } from '@/components/auth/ThemeToggle';
import { Button } from '@/components/ui/Button';
import { Input } from '@/components/ui/Input';
import { useAuthStore } from '@/stores/authStore';
import { getEquipmentList } from '@/services/inventoryService';
import { getMyRequests, submitBorrowRequest, returnBorrowRequest } from '@/services/requestService';
      ...
      <div className="min-h-screen relative overflow-hidden bg-gradient-to-br from-lime-50 via-cyan-50 to-lime-100 dark:from-slate-950 dark:via-purple-950 dark:to-slate-900 transition-colors duration-300">
        ...
        <div className="relative z-10 max-w-6xl mx-auto px-4 py-10">
          <div className="mb-8 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
              <h1 className="text-3xl font-bold text-lime-700 dark:text-white">Inventory Dashboard</h1>
              <p className="text-slate-600 dark:text-slate-300 mt-1">
                Signed in as <span className="font-semibold">{user?.username}</span> ({user?.role})
              </p>
            </div>

            <Button variant="secondary" onClick={onSignOut} icon={<LogOut className="w-4 h-4" />}>
              Log out
            </Button>
          </div>
          ...
          <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
            <section className="rounded-2xl border border-slate-200/80 dark:border-slate-700/60 bg-white/90 dark:bg-slate-900/80 backdrop-blur-lg p-6">
              <div className="flex items-center justify-between mb-4">
                <h2 className="text-xl font-bold text-slate-900 dark:text-white">Equipment Catalog</h2>
              </div>
                    ...
                    >
                      <div className="flex items-start justify-between gap-3">
                        ...
                        {request.status === 'approved' ? (
                          <Button size="sm" variant="secondary" onClick={() => onReturn(request.id)}>
                            Return
                          </Button>
                        ) : null}
                      </div>
